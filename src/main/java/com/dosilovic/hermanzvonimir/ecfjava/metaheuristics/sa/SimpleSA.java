...
public class SimpleSA<T> extends AbstractSA<T> {
    ...
    public SimpleSA(double desiredFitness, double desiredPrecision, IProblem<T> problem, IMutation<T> mutation,
        ICoolingSchedule outerCoolingSchedule, ICoolingSchedule innerCoolingSchedule) {
        super(desiredFitness, desiredPrecision, problem, mutation, outerCoolingSchedule, innerCoolingSchedule);
    }

    @Override protected Solution<T> onOuterTemperatureStart(Solution<T> currentSolution, Solution<T> bestSolution,
        double outerTemperature) {
        return currentSolution;
    }

    @Override protected Solution<T> onInnerTemperatureStart(Solution<T> currentSolution, Solution<T> bestSolution,
        double outerTemperature, double innerTemperature) {
        return currentSolution;
    }

    @Override protected Solution<T> selectNextSolution(Solution<T> currentSolution, Solution<T> neighbourSolution,
